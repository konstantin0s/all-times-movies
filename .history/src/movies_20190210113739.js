...
var dramaMov =  movies.filter(function(movie) {
    // set all variables at the top, because of variable hoisting
    var i,
        sum = 0, // try to use informative names, not "num"
        len = parseInt(movie.genre.length); // cache arr.length because accessing it is usually expensive
        console.log(len);
    for (i = 0; i < len; i++) { // arrays start at 0 and go to array.length - 1
        sum += movies[i].genre; // short for "sum = sum + arr[i];"
